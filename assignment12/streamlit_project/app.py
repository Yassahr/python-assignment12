...
st.subheader("You dont know")  # Slightly smaller than header — useful for structure
...
#button are stateless so there has to be a script that runs immediately there is not "clicked" button state it is only clicked in that instance
    #so anything that needs to happen from the button being click needs to run or be saved in that instance in order to maintain continuity
